...
- 사설 IP·루프백·링크로컬 주소(예: `127.0.0.1`, `169.254.169.254`, `192.168.x.x`, `localhost`)를 가리키는 URL은 실제로 그 주소에 요청을 보내지 않고 위와 같은 에러로 거부된다. 리다이렉트를 따라가는 경우에도 각 홉마다 같은 검사를 적용한다. 이 보장은 호스트명을 검사한 시점이 아니라 실제로 연결이 이루어지는 IP를 기준으로 하며, 검사와 연결 사이에 DNS 응답이 바뀌는 경우(DNS 재바인딩)에도 깨지지 않는다.
...
- **서버는 사설·루프백·링크로컬 등 내부망 주소로는 요청을 보내지 않는다(SSRF 방지)** — 임의의 사용자 제공 URL을 서버가 대신 가져오는 구조이므로, 이 검증이 없으면 이 도구가 내부망 접근 통로로 악용될 수 있다. 취향이 아니라 보안 정합성 문제로 간주해 협의 없이 확정한다. 호스트명을 한 번 검사하는 것만으로는 이 요구사항을 만족하지 않는다 — 검사와 실제 연결 사이에 DNS 응답이 바뀌는 공격(DNS 재바인딩)까지 포함해, 실제로 연결되는 IP가 내부망이 아님을 보장해야 한다.
